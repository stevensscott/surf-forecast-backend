...
# air_temp_seven_am=wind_info["hourly"]["windspeed_10m"][13]
# wind_speed_twelve_pm=wind_info["hourly"]["windspeed_10m"][18]
# wind_speed_five_pm=wind_info["hourly"]["windspeed_10m"][23]
...
puts "Today's Second Beach Surf Forecast- Middletown, RI"
puts "7AM Forecast- Wave Height: #{metric_to_feet(wave_seven_am)} feet. Wind Direction: #{convert_wind_from_cardinal_to_direction(wind_direction_seven_am)}. Wind Speed: #{wind_speed_seven_am} mph."
puts "12PM Forecast- Wave Height #{metric_to_feet(wave_twelve_pm)} feet. Wind Direction: #{convert_wind_from_cardinal_to_direction(wind_direction_twelve_pm)}. Wind Speed: #{wind_speed_twelve_pm} mph."
puts "5PM Forecast- Wave Height #{metric_to_feet(wave_five_pm)} feet. Wind Direction: #{convert_wind_from_cardinal_to_direction(wind_direction_five_pm)}. Wind Speed: #{wind_speed_five_pm} mph."


#Incorporate the TTY table 
#Write data to DB
#Add logic to suggest board to ride, which wetsuits etc based on the authenticated users
#Add csv import utility to bulk add actual observed conditons



#Create the condition here via POST